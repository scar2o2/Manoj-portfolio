...
import TerminalBody from "../appBodies/TerminalBody";

const TerminalTab = ({tabName}) => {
  ...
  useEffect(() => {
    ...
    if (!taskbar.querySelector('#TerminalTask')) {
      ...
      minTask.appendChild(document.createTextNode(" Terminal"));

      minTask.addEventListener("click", () => {
        const win = document.querySelector('.terminalWindow');
        if (win) {
          win.style.display =
  ...
  const closeTab = () => {
    const TerminalTask = document.querySelector('#TerminalTask');
    if (TerminalTask) TerminalTask.remove();
    setWhtTab(prev => prev.filter(item => item !== "terminal"));
  };
