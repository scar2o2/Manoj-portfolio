...
import AboutBody from "../appBodies/AboutBody";
...
const AboutTab = ({tabName}) => {
  ...
  useEffect(() => {
    ...
    if (!taskbar.querySelector('#AboutTask')) {
      ...
      minTask.appendChild(document.createTextNode(" About"));

      minTask.addEventListener("click", () => {
        const win = document.querySelector('.aboutWindow');
        if (win) {
          win.style.display =
  ...
  const closeTab = () => {
    const AboutTask = document.querySelector('#AboutTask');
    if (AboutTask) AboutTask.remove();
    setWhtTab(prev => prev.filter(item => item !== "about"));
  };
