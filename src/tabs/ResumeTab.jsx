import ResumeBody from "../appBodies/ResumeBody";
import { TabContext } from "../apps/Tab"
import { useContext, useEffect, useState } from "react";

const ResumeTab = ({tabName}) => {
  ...
  useEffect(() => {
    ...
    if (!taskbar.querySelector('#ResumeTask')) {
      ...
      minTask.appendChild(document.createTextNode("Resume"));

      minTask.addEventListener("click", () => {
        const win = document.querySelector('.resumeWindow');
        if (win) {
          win.style.display =
  ...
  const closeTab = () => {
    const ResumeTask = document.querySelector('#ResumeTask');
    if (ResumeTask) ResumeTask.remove();
    setWhtTab(prev => prev.filter(item => item !== "resume"));
  };
  ...
  return (
    <div className="resumeWindow window" onMouseDown={() => bringToFront(tabName)} style={{ top: `${offset.top}px`, left: `${offset.left}px`, zIndex: zOrders[tabName] || 1 }}>
