...
import ContactBody from "../appBodies/ContactBody";

const ContactTab = ({tabName}) => {
  ...
  useEffect(() => {
    ...
    if (!taskbar.querySelector('#ContactTask')) {
      ...
      minTask.appendChild(document.createTextNode("Contact"));
  ...
  const closeTab = () => {
    const ContactTask = document.querySelector('#ContactTask');
    if (ContactTask) ContactTask.remove();
    setWhtTab(prev => prev.filter(item => item !== "contact"));
  };
  ...
  return (
    <div className="contactWindow window" onMouseDown={() => bringToFront(tabName)} style={{ top: `${offset.top}px`, left: `${offset.left}px` , zIndex: zOrders[tabName] || 1 }}>
